...
| `STATUS` | `SERVICEORDERS.STATUS` | `TROUBLE_TICKETS.STATUS` | **Category** | ⭐ **HIGH** |
| `TROUBLE_TICKET_STATUS` | `SERVICEORDERS.STATUS` | `TROUBLE_TICKETS.STATUS` | **Category** | Medium |
...
CAMVIO.PUBLIC.TROUBLE_TICKETS.STATUS → STATUS (and TROUBLE_TICKET_STATUS for trouble tickets)
...
- [ ] Set `STATUS` → **Category** (unified status field for both types)
- [ ] Set `TROUBLE_TICKET_STATUS` → **Category** (same as STATUS, kept for consistency)
